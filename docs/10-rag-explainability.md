...
`/v1/explain` is the only place `core.ollama_client`'s `LLM_MODEL` constant is used for generation (as opposed to `EMBED_MODEL`, used only by `embeddings/generate_embeddings.py`). Because `core/ollama_client.py` resolves and validates the Ollama host **at import time** (see [04 · Core Infrastructure §4.2](./04-core-infrastructure.md#42-ollama-host-resolution--coreollama_clientpy)), and `app.py` imports `routers.rag` unconditionally at startup, **an unreachable Ollama host will prevent the entire application from starting**, not just degrade the `/v1/explain` endpoint.
