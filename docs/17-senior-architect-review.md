...
**Update**: the startup-fragility issues below (the `Optional` import crash, the Ollama-at-import-time crash, and the Milvus-at-import-time crash) have all been fixed — see [16 · Known Issues](./16-known-issues-tech-debt.md) for the current, verified state. This review is kept largely as originally written since the architectural critique (import-driven startup sequencing, no auth model beyond a shared secret, zero caching/indexes, disconnected phases) remains a fair characterization of what's genuinely still true, with inline ✅ notes added where something specific has since been fixed.

**Bottom line up front (as originally written; see inline updates)**: Velar has good architectural instincts in places — a genuine confidence-wall philosophy, a real memory/trust state machine, a properly-designed grounded RAG pipeline — but it is not yet production-ready in the full sense: no authorization model beyond a single shared secret (now at least correctly *enforced*, not hardcoded), zero caching, zero database indexes, no dependency injection beyond one auth check, and about half of the "15 phases" were disconnected from the live system (now reachable via `/v1/pipelines/*`, though not yet scheduled). Treat this review as a punch list as much as an analysis.
...
sequenceDiagram
    ...
    Note over Cfg: Missing var → crash HERE, before anything else (unchanged — this is correct fail-fast behavior)
    App->>Rt: from routers import v1, memory, analytics, rag, pipelines
    Rt->>Rt: routers.rag imports rag.generator, embeddings.generate_embeddings
    Note over Oll: ✅ FIXED — get_ollama_host() no longer called at import time; resolution deferred to first RAG/embedding request
    Rt->>Rt: routers.rag imports milvus.search_vectors → milvus.insert_vectors
    Note over Rt: ✅ FIXED — VectorStoreManager no longer opens its own connection at import; it reads database.milvus.vector_db.client lazily
    App->>App: FastAPI(...), rate limiter, Prometheus, include_router x7
    ...
    LS->>Milvus: vector_store.ensure_collections() — creates/loads the Milvus collection, now here instead of at import time
    LS-->>OS: yield — now serving traffic
```

**Architectural assessment (original, now resolved)**: three separate subsystems (`core.config`, `core.ollama_client`, `milvus.insert_vectors`) used to each independently abort the entire process *during import*, before FastAPI's own lifespan even began — meaning "the app failed to start" could mean several different things, none producing a clean, actionable startup error. The Ollama and Milvus import-time resolution has been moved out of the import path: Ollama host resolution now happens lazily on first RAG/embedding request (see [04 · Core Infrastructure §4.2](./04-core-infrastructure.md#42-ollama-host-resolution--coreollama_clientpy)), and Milvus collection setup now happens explicitly inside `lifespan` via `vector_store.ensure_collections()`, right after `vector_db.connect()` — exactly the fix this section originally recommended. `core.config`'s fail-fast-on-missing-env-var behavior is intentionally unchanged, since that's correct, not fragile.
...
| `MILVUS_URI` | Yes | — | ✅ **Fixed** — `milvus/insert_vectors.py` no longer reads `MILVUS_URI` itself at all; it delegates to `database.milvus.vector_db`, which reads it via `settings` |
| `OLLAMA_URI` | No | `None` | Yes, takes precedence if set |
| `OLLAMA_HOSTS` | No | `None` | Yes, fallback failover list |
| `EMBED_MODEL` | Yes | — | Yes |
| `LLM_MODEL` | Yes | — | Yes |
| `VELAR_API_KEY` | Yes | — | ✅ **Fixed** — `core/security.py` now reads `settings.VELAR_API_KEY` |

**Previously, two of eight configured variables were effectively decorative** — declared, validated as required at startup, and then ignored by the code that should consume them. Both are fixed: `milvus/insert_vectors.py` no longer bypasses `settings` (it no longer owns its own client at all), and `core/security.py` reads the real configured key.

`docker-compose_production.yaml` previously compounded this by setting `MONGO_URI`/`MONGO_DB_NAME`/`MILVUS_HOST`/`MILVUS_PORT` — none of which matched `Settings`' actual field names. ✅ Fixed — it now sets `MONGODB_URI`/`MONGODB_DB_NAME`/`MILVUS_URI`/`OLLAMA_URI`/`OLLAMA_HOSTS`/`EMBED_MODEL`/`LLM_MODEL`/`VELAR_API_KEY`, matching `Settings` exactly, sourced via `${VAR:?required}` substitution rather than hardcoded values.

## 5. Configuration Loading

Configuration loading is **eager and centralized** for the happy path (`Settings()` instantiated once at `core/config.py` import time, fail-fast on missing required fields) — a good pattern in principle. Previously three separate places in the codebase read configuration a different way; two of those are now fixed:

1. ✅ `milvus/insert_vectors.py` — previously `os.getenv("MILVUS_URI", ...)` directly; now delegates entirely to `database.milvus.vector_db` (which reads `settings`).
2. ✅ `core/security.py` — previously a hardcoded literal; now reads `settings.VELAR_API_KEY`.
3. `scripts/mock_seeder.py` — still a fully hardcoded `mongodb://localhost:27017`, ignoring configuration entirely (acceptable for a throwaway dev script, not part of the live app's request path).
...
sequenceDiagram
    ...
    else not secrets.compare_digest(header, settings.VELAR_API_KEY)
...
This is **single-factor, single-secret, static-key authentication** — there is no user model, no per-client key issuance, no key expiry, no OAuth2/JWT, no rotation mechanism beyond editing `.env`/deployment config and restarting (rotation itself now actually works, see above). ✅ The comparison now uses `secrets.compare_digest` (constant-time) instead of plain `!=` — this closed the timing side-channel this section originally flagged.
...
- **`routers/v1.py`'s `categorize_transaction`** breaks the pattern by embedding real business logic — regex amount extraction and a direct Mongo write — directly in the controller, rather than delegating to a service. The three runtime bugs this used to have (a `.get()` call on a Pydantic model, a `time.timezone.utc` typo, and placeholder object references written to Mongo) are fixed, but the architectural critique stands: this logic would be better placed in a service/engine, and its being in the router is likely *why* it went unfinished for as long as it did (no clear ownership boundary).
...
- **Vector store**: `pymilvus.MilvusClient` — ✅ **fixed**: previously two independent client instances existed (`database/milvus.py`'s `vector_db`, and `milvus/insert_vectors.py`'s own eagerly-connected client), reading configuration through two different mechanisms. `milvus/insert_vectors.py`'s `VectorStoreManager` no longer opens its own connection — it delegates to `vector_db` via a `client` property, so there is now exactly one Milvus client per process, configured exactly once through `settings.MILVUS_URI`.
...
|---|---|---|
| ✅ Hardcoded API key, config ignored | Fixed | `core/security.py` now reads `settings.VELAR_API_KEY` — rotating the configured key works |
| No authorization model | **Still open** | Any valid key grants full, undifferentiated access to all data and all merchants — no per-caller scoping anywhere. Genuine feature work (multi-tenancy), not a bug fix — on the roadmap |
| ✅ Plaintext credential committed to git | Fixed going forward | `docker-compose_production.yaml` now sources all values via `${VAR:?required}` substitution — no credential is committed. The previously-committed credential is still in git history and must be rotated on the actual MongoDB server by whoever operates this deployment |
| ✅ Non-constant-time key comparison | Fixed | `core/security.py` now uses `secrets.compare_digest` instead of `!=` |
